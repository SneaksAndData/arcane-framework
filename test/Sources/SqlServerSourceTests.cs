...
namespace Arcane.Framework.Tests.Sources
{
    public class SqlServerSourceTests : IClassFixture<ServiceFixture>, IClassFixture<AkkaFixture>
    {
        ...
        {
            var result = await Source.FromGraph(SqlServerSource.Create(this.connectionString, "dbo", nameof(SqlServerSourceTests), "test"))
        ...
        {
            var schema = SqlServerSource.Create(this.connectionString, "dbo", nameof(SqlServerSourceTests), "test").GetParquetSchema();

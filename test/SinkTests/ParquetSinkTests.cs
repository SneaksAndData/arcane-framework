...
namespace Arcane.Framework.Tests.SinkTests
{
    public class ParquetSinkTests : IClassFixture<AkkaFixture>, IClassFixture<ServiceFixture>
    {
        ...
        public async Task ParquetSinkWrites(int blocks, int rowGroupsPerBlock, bool createSchemaFile, bool dropCompletionToken)
        {
            ...
            this.serviceFixture.MockBlobStorageService.Setup(mb => mb.SaveBytesAsBlob(It.IsAny<BinaryData>(), It.Is<string>(p => p.Contains(pathString)), It.IsAny<string>(), It.IsAny<bool>()))
                .ReturnsAsync(new UploadedBlob());

            await Source.From(Enumerable.Range(0, blocks).Select(_ => columns.ToList())).RunWith(ParquetSink.Create(parquetSchema: schema, storageWriter: this.serviceFixture.MockBlobStorageService.Object, parquetFilePath: $"tmp@{pathString}", rowGroupsPerFile: rowGroupsPerBlock, createSchemaFile: createSchemaFile, dropCompletionToken: dropCompletionToken), this.akkaFixture.Materializer);

            this.serviceFixture.MockBlobStorageService.Verify(mb => mb.SaveBytesAsBlob(It.IsAny<BinaryData>(), It.Is<string>(path => path.Contains(pathString)), It.Is<string>(fn => fn.StartsWith("part-")),It.IsAny<bool>()), createSchemaFile ? Times.Exactly(blocks / rowGroupsPerBlock + 2) : Times.Exactly(blocks / rowGroupsPerBlock + 1));
            this.serviceFixture.MockBlobStorageService.Verify(mb => mb.SaveBytesAsBlob(It.IsAny<BinaryData>(), It.Is<string>(path => path.Contains(pathString)), It.Is<string>(fn => fn.StartsWith("schema-")), It.IsAny<bool>()), createSchemaFile ? Times.Exactly(1) : Times.Exactly(0));

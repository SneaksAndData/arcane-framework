    ...
    public class MultilineJsonSinkTest : IClassFixture<AkkaFixture>, IClassFixture<ServiceFixture>
    {
        ...
        public async Task MultilineJsonSinkWrites(int files, int rowsPerFile, bool dropsCompletion)
        {
            ...
            var mockIn = Enumerable
                .Range(0, files)
                .Select(_ => Enumerable.Range(0, rowsPerFile).Select(ix => JsonSerializer.Deserialize<JsonElement>(JsonSerializer.Serialize(new { Value = ix }))))
                .ToList();

            this.serviceFixture.MockBlobStorageService.Setup(mb => mb.SaveBytesAsBlob(It.IsAny<BinaryData>(), It.Is<string>(p => p == mockPath), It.IsAny<string>(), It.IsAny<bool>()))
                .ReturnsAsync(new UploadedBlob());

            await Source
                .From(mockIn)
                .Select(v => v.ToList())
                .RunWith(MultilineJsonSink.Create(this.serviceFixture.MockBlobStorageService.Object, mockPath, mockSchema, "data", "schema", dropsCompletion), this.akkaFixture.Materializer);

            foreach (var _ in Enumerable.Range(0, files))
            {
                this.serviceFixture.MockBlobStorageService.Verify(mb => mb.SaveBytesAsBlob(
                    It.Is<BinaryData>(bd => (bd.ToArray().Length > 0 ? bd.ToString() : string.Empty).Split(Environment.NewLine, StringSplitOptions.None).Length - 1 == rowsPerFile),

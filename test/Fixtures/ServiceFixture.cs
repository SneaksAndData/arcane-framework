...
namespace Arcane.Framework.Tests.Fixtures
{
    public class ServiceFixture
    {
        ...
        public ServiceFixture()
        {
            ...
            {
                sqlCon.Open();
                var createDbCmd = new SqlCommand("IF NOT EXISTS (SELECT * FROM sys.databases WHERE name = 'arcane') BEGIN CREATE DATABASE arcane; alter database Arcane set CHANGE_TRACKING = ON (CHANGE_RETENTION = 2 DAYS, AUTO_CLEANUP = ON); END;", sqlCon);
        ...
        public string PrepareSqlServerDatabase(string tableName)
        {
            ...
            {
                sqlCon.Open();
                var createTableCmd = new SqlCommand($"use arcane; drop table if exists dbo.{tableName}; create table dbo.{tableName}(x int not null, y int)", sqlCon);
                createTableCmd.ExecuteNonQuery();

                var createPKCmd = new SqlCommand($"use arcane; alter table dbo.{tableName} add constraint pk_{tableName} primary key(x); ", sqlCon);
                ...
                var fillRowsCmds = Enumerable.Range(0, 100).Select(ix => new SqlCommand($"use arcane; insert into dbo.{tableName} values ({ix}, {ix + 1})", sqlCon));
        ...
        public void CreateTable(string connectionString, string tableName, Dictionary<string, string> columns, string pkColumnName)
        {
            using var sqlCon = new SqlConnection(connectionString);
            sqlCon.Open();
            var cols = string.Join(",", columns.Select(c => $"{c.Key} {c.Value}"));
            var createTableCmd = new SqlCommand($"use arcane; drop table if exists dbo.{tableName}; create table dbo.{tableName} ({cols})", sqlCon);
            createTableCmd.ExecuteNonQuery();

            var createPKCmd = new SqlCommand($"use arcane; alter table dbo.{tableName} add constraint pk_{tableName} primary key([{pkColumnName}]); ", sqlCon);
            createPKCmd.ExecuteNonQuery();
